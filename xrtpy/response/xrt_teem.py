...
def xrt_teem(hdr1, data1, hdr2, data2, binfac = 1, trange = None, 
    ...
    if binfac > 1:
        ...
        for i in range(ns[0]):
            for j in range(ns[1]):
                d1tmp[binfac*i:binfac*i+binfac+1,
                    binfac*j:binfac*j+binfac+1] = d1[i,j]
                d2tmp[binfac*i:binfac*i+binfac+1,
                    binfac*j:binfac*j+binfac+1] = d2[i,j]
                mtmp[binfac*i:binfac*i+binfac+1,
                    binfac*j:binfac*j+binfac+1] = msk[i,j]
        # alternative method
    ...
    dwl = (wl[1:] - wl[:-1]).mean()  # assumes uniform grid
    eVe = tresp1.ev_per_electron
    gain = tresp1.ccd_gain_right
    sldang = tresp1.solid_angle_per_pixel
    #e2dn = (h.to(u.eV*u.s)*c.to(u.angstrom/u.s)*sldang*dwl/(wl*eVe*gain)
    #        ).value
    ...
    p = hdr1['PLATESCL']
    l = 6.96000E10/angular_radius(hdr1['DATE_OBS']).value
    em2vem = (p*l)**2
    flux1 /= em2vem
    flux2 /= em2vem
    ratio = flux1/flux2
    ...
    for i,m in enumerate(model_ratio[1:]):
        n = ((data_ratio >= min(model_ratio[i],m)) & (data_ratio <=
            max(model_ratio[i],m)))
        if np.sum(n) > 0:
            ok_num[n] = i + 1
            ok_cnt[n] += 1
            #print(f'i = {i+1}, t = {np.log10(t[i+1]):.2F}, n.sum() = '
            #        f'{n.sum():5d}, ok_cnt[n].max() = {ok_cnt[n].max()}') 
...
def deriv(x,y):
    ...
    dydx0 = (y0[0]*(x01[0] + x02[0])/(x01[0]*x02[0]) 
            - y1[0]*x02[0]/(x01[0]*x12[0]) + y2[0]*x01[0]/(x02[0]*x12[0]))
